...
- [x] 3.1 [TDD:RED→GREEN] `app/composables/usePlatos.ts`: `useAsyncData` from Supabase, sort by `puesto`. (CN-006)
- [x] 3.2 [TDD:RED→GREEN] `app/components/PlatoForm.vue`: fields per CRUD-002, Spanish labels, validation. (CRUD-002)
- [x] 3.3 [TDD:RED→GREEN] `app/components/PlatosTable.vue`: sortable columns, toggle disponible, delete confirm. (CRUD-001, CRUD-004, CRUD-005)
- [x] 3.4 [TDD:RED→GREEN] `app/pages/cocina/carta.vue`: list + create/edit/delete flows. (CRUD-001 to CRUD-005)
- [x] 3.5 [TDD:RED→GREEN] `app/composables/useMenuDiario.ts`: join config+items for current day. (MD-004, MD-005)
- [x] 3.6 [TDD:RED→GREEN] `app/components/MenuDiarioEditor.vue`: day selector + 5-section dish manager + add/remove. (CMD-001 to CMD-005)
- [x] 3.7 [TDD:RED→GREEN] `app/pages/cocina/menu-diario.vue`: day list + editor integration. (CMD-001 to CMD-005)
- [x] 3.8 [TDD:RED→GREEN] `app/composables/useEventos.ts`: `useAsyncData`, active only, fecha ASC. (EG-001)
- [x] 3.9 [TDD:RED→GREEN] `app/components/EventoForm.vue` + `EventosTable.vue`: form with date picker + sortable list. (CEV-001, CEV-002)
- [x] 3.10 [TDD:RED→GREEN] `app/pages/cocina/eventos.vue`: list + create/edit/delete + toggle activo. (CEV-001 to CEV-004)
- [x] 3.11 [MIGRATE] `app/pages/carta.vue`: replace mock import → `usePlatos()`. (CN-006)
- [x] 3.12 [MIGRATE] `app/pages/menu-diario.vue`: replace mock → `useMenuDiario()` with variable pricing. (MD-001, MD-004, MD-005)
- [x] 3.13 [MIGRATE] `app/pages/eventos.vue`: replace mock → `useEventos()`, only active+future. (EG-001)
- [x] 3.14 [GATE] Public pages SSR 200; CRUD forms render; vitest green on composables+components.
